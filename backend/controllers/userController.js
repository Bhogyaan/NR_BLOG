...
const getAdminRealtimeDashboard = async (req, res) => {
  try {
      ...
      Post.aggregate([
        {
          $group: {
            ...
            totalShares: { $sum: 0 }, // Shares not implemented in schema, default to 0
          },
        },
      ]),
      Post.aggregate([
    ...
    for (let i = 0; i < days; i++) {
      ...
      const existingData = activityData.find((d) => d._id === dateStr) || { posts: 0, likes: 0, comments: 0 };
    ...
    for (let i = 0; i < days; i++) {
      const date = new Date(startDate.getTime() + i * 24 * 60 * 60 * 1000);
      const dateStr = date.toISOString().split('T')[0];
      const existingData = userGrowth.find((d) => d._id === dateStr) || { newUsers: 0 };
    ...
    for (let i = 0; i < days; i++) {
      const date = new Date(startDate.getTime() + i * 24 * 60 * 60 * 1000);
      const dateStr = date.toISOString().split('T')[0];
      const existingData = postGrowth.find((d) => d._id === dateStr) || { newPosts: 0 };
    ...
    res.status(200).json({
      ...
      recentPosts: recentPosts.map((post) => ({
        ...
        postedBy: post.postedBy || { username: "Unknown", profilePic: "", isBanned: false },
      })),
      engagementRate,
    });
  } catch (error) {
    console.error("Error in getAdminRealtimeDashboard:", error.message);
    res.status(500).json({
